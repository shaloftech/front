...
const AdminTradeControl = () => {
  ...
  useEffect(() => {
    const fetchClientsAndAgents = async () => {
      try {
        ...
        setUsers(clientResponse.data);
        setAgents(agentResponse.data);
        setFilteredUsers(clientResponse.data);
      } catch (error) {
  ...
  const groupedClients = users.reduce((acc, user) => {
    if (!acc[user.agentUID]) {
      acc[user.agentUID] = [];
    }
    acc[user.agentUID].push(user);
    return acc;
  }, {});
            ...
            {Object.keys(groupedClients).map((agentUID) => (
              <React.Fragment key={agentUID}>
                ...
                {expandedAgents[agentUID] &&
                  groupedClients[agentUID].map((user) => (
