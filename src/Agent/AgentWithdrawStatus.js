  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch send requests for the logged-in agent
        const sendResponse = await axios.get(
          `https://trustcoinfx.trade/api/send-requests-for-agent/${agentId}`
        );

        // Fetch clients assigned to the logged-in agent
        const userResponse = await axios.get(
          `https://trustcoinfx.trade/api/clients-by-agent/${agentId}`
        );
